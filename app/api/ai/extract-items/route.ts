...
複数の商品が含まれる場合はitemsを配列で。商品情報が見つからない場合は空配列。プロファイリングは必ず実施する。`;
  ...
  try {
    // 静的な systemPrompt (買取査定指針 + JSON フォーマット仕様)
    // Haiku 4.5 の最小キャッシュ要件 4096 tokens に概ね達する想定。
    // 未達の場合は API がエラーを返さず単にキャッシュ無し動作になるので無害。
  ...
  // 顧客プロファイル・追加買取サジェストを inquiries テーブルに永続化
  // （以前は揮発で毎回 AI 抽出をやり直す必要があった。ダッシュボード集計にも必要）
  const supabase = createServiceClient();
  if (customerProfile || suggestedItems.length > 0 || approachHint) {
    try {
      ...
      await (supabase as any)
        ...
        .update({
          customer_profile: customerProfile,
          suggested_items: suggestedItems.length > 0 ? suggestedItems : null,
          approach_hint: approachHint || null,
          profile_extracted_at: new Date().toISOString(),
        })
        .eq("id", body.inquiry_id);
    } catch (e) {
      console.error("extract-items: failed to persist customer profile", e);
      // プロファイル保存失敗は致命的ではないため処理続行
    }
  }
