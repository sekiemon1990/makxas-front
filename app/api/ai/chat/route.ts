...
import { buildUsageEnvelope, buildAiEvent } from "@/lib/ai/event";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      // コスト追跡: tool_use ループの各 iteration を記録 (ADR-0029 AiEventContract を meta に埋め込む)
      const usageEnvelope = buildUsageEnvelope(response.usage, MODEL);
      const aiEvent = buildAiEvent({
        conversationId,
        usage: usageEnvelope,
        action: response.stop_reason === "end_turn" ? "executed" : "pending",
      });
      await logAiUsage({
        category: "chat",
        model: MODEL,
        ...
        meta: { iteration: i + 1, stop_reason: response.stop_reason, ai_event: aiEvent },
      });
